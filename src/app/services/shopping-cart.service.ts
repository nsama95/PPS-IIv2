...
export class ShoppingCartService {
    ...
    return this.db
      ...
      .pipe(
        map((item) => {
          ...
          item.forEach((i: { product: IProducto; quantity: number }) => {
  ...
  async addToCart(product: IProducto) {
    this.updateItemQuantity(product, 1);
  }

  async removeFromCart(product: IProducto) {
    this.updateItemQuantity(product, -1);
  }
  ...
  private async updateItemQuantity(product: IProducto, change: number) {
    let cartId = await this.getOrCreateCartId();
